...
export const crearOrdenCarrito = async (req, res) => {
  try {
    ...
    const productosDB = await product.find({ _id: { $in: idsProductos } }); //$in buscar varios elementos a la vez cuando tenemos una lista de identificadores.
    ...
    if (notification.type === "payment") {
      ...
      if (payment && payment.status === "approved") {
        ...
        if (pedido.status === "pendiente") {
          ...
          console.log(`El pedido ${pedidoId} ya fue procesado. Estado actual: ${pedido.status}`);
        }
